...
#SK: Removing everything related to use_FiLM
class PGD():
    def __init__(self, eps, steps=7, alpha=None, loss_fn=None, targeted=False):
        ...
        #self.alpha = 0.008 #set alpha to 2/255
        self.targeted = targeted
        self.loss_fn = loss_fn if loss_fn else nn.CrossEntropyLoss(reduction="sum")     

    #SK: removed _lambda=None,
    def attack(self, model, x, labels=None, targets=None,  idx2BN=None):
        '''
        Args:
        ...
        for t in range(self.steps):
            #To generate adversary we make w_noise as True, meaning the loss will be
            # computed on logit response under noisy model.
            logits_adv = model(x_adv, 1.0, w_noise=True, idx2BN=idx2BN)
            if self.targeted:
