...
class WideResNet(nn.Module):
    ...
    def forward(self, x):
        # print('x:', x.size(), torch.max(x), torch.min(x))
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        ...
        # print('out:', out.size())
        out = F.avg_pool2d(out, 24) # 24 for (96,96) STL images
        out = out.view(-1, self.nChannels)
        return self.fc(out)

def WRN40_2():
    return WideResNet(depth=40, num_classes=10, widen_factor=2, dropRate=0.3)
