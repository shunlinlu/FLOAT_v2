...
import numpy as np
#SK: from models.slimmable_ops used noise_SlimmableConv2d
from models.slimmable_ops import noise_SlimmableConv2d, SlimmableLinear, width_mult_list, SwitchableBatchNorm2d
...
class SlimmableBasicBlockOAT(nn.Module):
    ...
    def __init__(self, in_planes_list, out_planes_list, stride=1, use2BN=True, FiLM_in_channels=1):
        ...
        self.conv1 = noise_SlimmableConv2d(in_planes_list, out_planes_list, kernel_size=3, stride=stride, padding=1, bias=False)
        # self.bn1 = nn.BatchNorm2d(out_planes)
        self.bn1 = Norm2d(out_planes_list)
        # self.conv2 = nn.Conv2d(out_planes, out_planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv2 = noise_SlimmableConv2d(out_planes_list, out_planes_list, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(out_planes)
        self.bn2 = Norm2d(out_planes_list)

        if stride != 1 or list(in_planes_list) != list(out_planes_list):
            self.mismatch = True
            self.conv_sc = noise_SlimmableConv2d(in_planes_list, out_planes_list, kernel_size=1, stride=stride, bias=False)
    ...
    def forward(self, x, _lambda, w_noise=True, idx2BN=None):
        ...
        if self.mismatch:
            if self.use2BN: 
                out += self.bn_sc(self.conv_sc(x, _lambda, w_noise), idx2BN)
            else:
                out += self.bn_sc(self.conv_sc(x, _lambda, w_noise))
        else:
            out += x
        out = F.relu(out)
        # print(out.size())
...
class SlimmableResNet34OAT(nn.Module):
    def __init__(self, num_classes=10, FiLM_in_channels=1, use2BN=True):
        ...
        self.conv1 = noise_SlimmableConv2d([3 for _ in width_mult_list], list64,
        ...
        self.bundle1 = nn.ModuleList([
            SlimmableBasicBlockOAT(list64, list64, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list64, list64, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list64, list64, stride=1, use2BN=use2BN),
        ])
        self.bundle2 = nn.ModuleList([
            SlimmableBasicBlockOAT(list64, list128, stride=2, use2BN=use2BN),
            SlimmableBasicBlockOAT(list128, list128, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list128, list128, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list128, list128, stride=1, use2BN=use2BN),
        ])
        self.bundle3 = nn.ModuleList([
            SlimmableBasicBlockOAT(list128, list256, stride=2, use2BN=use2BN),
            SlimmableBasicBlockOAT(list256, list256, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list256, list256, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list256, list256, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list256, list256, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list256, list256, stride=1, use2BN=use2BN),
        ...
        self.bundle4 = nn.ModuleList([
            SlimmableBasicBlockOAT(list256, list512, stride=2, use2BN=use2BN),
            SlimmableBasicBlockOAT(list512, list512, stride=1, use2BN=use2BN),
            SlimmableBasicBlockOAT(list512, list512, stride=1, use2BN=use2BN),
        ])
...
#SK: changes to slimmableBottlenectkBlockOAT is TBD
