...
class BasicBlockOAT(nn.Module):
    ...
    def __init__(self, in_planes, mid_planes, out_planes, stride=1, use2BN=False):
        ...
        self.conv1 = noise_Conv2d(in_planes, mid_planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = Norm2d(mid_planes)
        self.conv2 = noise_Conv2d(mid_planes, out_planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = Norm2d(out_planes)

        if stride != 1 or in_planes != out_planes:
            self.mismatch = True
            self.conv_sc = noise_Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class ResNet34OAT(nn.Module):
    ...
    def __init__(self, num_classes=10, use2BN=False):
        ...
        self.conv1 = noise_Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        if self.use2BN:
        ...
        self.bundle1 = nn.ModuleList([
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN),
        ])
        self.bundle2 = nn.ModuleList([
            BasicBlockOAT(64, 128, 128, stride=2, use2BN=use2BN),
            BasicBlockOAT(128, 128, 128, stride=1, use2BN=use2BN),
            BasicBlockOAT(128, 128, 128, stride=1, use2BN=use2BN),
            BasicBlockOAT(128, 128, 128, stride=1, use2BN=use2BN),
        ])
        self.bundle3 = nn.ModuleList([
            BasicBlockOAT(128, 256, 256, stride=2, use2BN=use2BN),
            BasicBlockOAT(256, 256, 256, stride=1, use2BN=use2BN),
            BasicBlockOAT(256, 256, 256, stride=1, use2BN=use2BN),
            BasicBlockOAT(256, 256, 256, stride=1, use2BN=use2BN),
            BasicBlockOAT(256, 256, 256, stride=1, use2BN=use2BN),
            BasicBlockOAT(256, 256, 256, stride=1, use2BN=use2BN),
        ...
        self.bundle4 = nn.ModuleList([
            BasicBlockOAT(256, 512, 512, stride=2, use2BN=use2BN),
            BasicBlockOAT(512, 512, 512, stride=1, use2BN=use2BN),
            BasicBlockOAT(512, 512, 512, stride=1, use2BN=use2BN),
        ])
    ...
    def forward(self, x, _lambda=1.0, w_noise=True, idx2BN=None):
        out = self.conv1(x, _lambda, w_noise)
        if self.use2BN:
            out = self.bn1(out, idx2BN)
        else:
